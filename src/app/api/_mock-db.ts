...
const SEED_CONNECTED_EQUIPMENT: ConnectedEquipmentListItem[] = [
  {
    id: 'EQ-0001',
    name: '水素水サーバー',
    store_id: 'store-002',
...
const SEED_CONTROLLERS: Controller[] = [
  {
...
function buildControllerHistorySeed(controller: Controller): ControllerHistoryItem[] {
...
const SEED_CONTROLLER_HISTORY: Record<string, ControllerHistoryItem[]> = Object.fromEntries(
...
function buildControllerDeviceSummary(
...
function buildUsageControlRule(item: ConnectedEquipmentListItem): UsageControlRuleDisplay {
  ...
  for (const label of item.linked_contract_labels) {
    ...
    if (label.includes('都度')) {
      if (!contract_link_types.includes('per_use')) {
        contract_link_types.push('per_use');
      }
      per_use_option_type_label = label.replace(/都度利用$/, '').trim() || label;
      continue;
    }
...
function buildUsageControlRuleFromInput(
...
function buildLabelsFromRule(rule: EquipmentUsageControlRuleInput | null): string[] {
  if (!rule) return [];
  const labels: string[] = [];
...
function buildEquipmentHistorySeed(
  equipmentId: string,
  installedOn: string,
): EquipmentStatusHistoryItem[] {
...
const SEED_EQUIPMENT_HISTORY: Record<string, EquipmentStatusHistoryItem[]> = {
  'EQ-0001': [
    {
      id: 'EQ-0001-hist-004',
      occurred_at: '2026-02-15T10:00:00+09:00',
...
function createDb() {
  ...
  const db = {
    ...
    equipment: {
      ...
      getHistory(id: string): EquipmentStatusHistoryItem[] {
        this._seed();
        if (!this._rows.some((equipment) => equipment.id === id)) {
          return [];
        }
      ...
      bulkUpdateStatus(
        ...
        return uniqueIds.map((id) => {
          const index = this._rows.findIndex((equipment) => equipment.id === id);
          if (index === -1) {
            return { id, success: false, error: 'Equipment not found' };
          }
    ...
    controllers: {
      ...
      getConnectedDevices(controllerId: string): {
        ...
        if (!controller) {
          return { devices: [], summary: buildControllerDeviceSummary([]) };
        }

        const devices: ControllerConnectedDevice[] = db.equipment._rows
          .filter((equipment) => {
            const meta = db.equipment._metaById[equipment.id];
            if (meta?.controller_id) {
              return meta.controller_id === controllerId;
            }
      ...
      getHistory(controllerId: string): ControllerHistoryItem[] {
        this._seed();
        if (!this._rows.some((row) => row.controller_id === controllerId)) {
          return [];
        }
      ...
      create(input: UpsertControllerRequest): ControllerDetail {
        ...
        return {
          ...row,
          device_count: 0,
          device_summary: buildControllerDeviceSummary([]),
        };
      },
      update(controllerId: string, input: PatchControllerRequest): ControllerDetail | undefined {
        this._seed();
        ...
        // No history append (Q3): history is seed/read-only.
        const { devices, summary } = this.getConnectedDevices(controllerId);
        return {
          ...nextRow,
          device_count: devices.length,
