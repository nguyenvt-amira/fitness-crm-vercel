...
/**
 * Update Y-07 brand master
 *
 * ブランド基本設定を更新する（本部のみ）
 */
export const patchCrmBrandsByCodeMutation = (options?: Partial<Options<PatchCrmBrandsByCodeData>>): UseMutationOptions<PatchCrmBrandsByCodeResponse, PatchCrmBrandsByCodeError, Options<PatchCrmBrandsByCodeData>> => {
    const mutationOptions: UseMutationOptions<PatchCrmBrandsByCodeResponse, PatchCrmBrandsByCodeError, Options<PatchCrmBrandsByCodeData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await Brands.patchCrmBrandsByCode({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
...
/**
 * Create Y-07 brand master
 *
 * ブランド基本設定を新規作成する（本部のみ）
 */
